...
def custom_score(game, player):
    ...
    opp_move = len(game.get_legal_moves(game.get_opponent(player)))

    return float(own_move - opp_move) 
...
def custom_score_2(game, player):
    ...
    return float(own_move - 2*opp_move) 
...
def custom_score_3(game, player):
    ...
    return float(2*own_move - opp_move) 
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        v = float("-inf")
        for m in game.get_legal_moves():
            tmp_score = self.min_value(game.forecast_move(m), depth-1, alpha, beta)
            if v < tmp_score:
                v = tmp_score
                best_move = m
            alpha = max(alpha, v)
